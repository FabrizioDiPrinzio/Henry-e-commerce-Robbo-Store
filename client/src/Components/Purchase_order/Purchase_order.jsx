		...
		<div className='outerContainer'>
			<div className="purchaseOrderContainer">
				...
				<div className='firstRow'>
					...
					<div className='purchaseOrderStatus'>
						...
						<h3>
							...
							<OverlayTrigger overlay={<Tooltip id="tooltip-Status">{'enCarrito → creada → pagada → entregada'}</Tooltip>}>
				...
				<div className='totalPrice'>
					<h3> Total: <span class="badge badge-primary">U$S {totalPrice ? totalPrice.price : 0}</span></h3>
				</div>
				...
				<div className='shippingData'>
					...
					aria-expanded={openShippingData}>
      				...
      				<Collapse in={openShippingData}>
      					...
      					<p>
							Nombre del Destinatario: <strong> {`${purchaseOrderData.recipient_name}`} </strong> <br />
							Apellido del Destinatario: <strong> {`${purchaseOrderData.recipient_lastname}`} </strong> <br />
							País de Entrega: <strong> {`${purchaseOrderData.country}`} </strong> <br />
							Ciudad de Entrega: <strong> {`${purchaseOrderData.city}`} </strong> <br />
							Dirección de Entrega: <strong> {`${purchaseOrderData.address}`} </strong> <br />
							Código Postal: <strong> {`${purchaseOrderData.postal_code}`} </strong> <br />
							Numero de Teléfono: <strong> {`${purchaseOrderData.phone_number}`} </strong> <br />
							Tipo de Envío: <strong> {`${purchaseOrderData.shipping_type}`} </strong> <br />
						</p>
						</div>
      				</Collapse>
				...
				<div className='paymentData'>
					...
					aria-expanded={openPaymentData}>
      				...
      				<Collapse in={openPaymentData}>
      					<div className='paymentInfo card card-body'>
							Descuentos o cuotas ...
						</div>
      				</Collapse>
